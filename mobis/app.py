...
===============================================
SWITCHED FROM vidsrc.to → vidsrc.me
  vidsrc.to blocks iframe embedding (X-Frame-Options: SAMEORIGIN)
  vidsrc.me allows iframe embedding and is actively maintained

EMBED ENDPOINTS (vidsrc.me):
  /embed/movie/{imdb_id}                         — movie player
  /embed/tv/{imdb_id}/{season}/{episode}          — TV episode
  /embed/tv/{imdb_id}                             — TV (defaults S1E1)

BROWSE API (vidsrc.me):
  /movies/new/{page}                             — new movies
  /movies/add/{page}                             — recently added movies
  /tvshows/new/{page}                            — new TV shows
  /tvshows/add/{page}                            — recently added TV
  /episodes/latest/{page}                        — latest episodes
...
# ── CHANGED: vidsrc.me allows iframes; vidsrc.to does NOT ────────────────────
...
def search_imdb(query):
    """IMDB free suggestion API — no key needed."""
...
def vapi(kind, type_or_path, page=1):
    """
    Fetches browse data from vidsrc.me API.

    kind         = 'movie' | 'tv' | 'episode'
    type_or_path = 'new' | 'add' | 'latest'

    vidsrc.me API endpoints:
      /movies/new/{page}
      /movies/add/{page}
      /tvshows/new/{page}
      /tvshows/add/{page}
    ...
    # Build correct vidsrc.me API URL
    if kind == "episode":
    ...
    try:
        ...
        data = r.json()
        # vidsrc.me returns: {"status":200,"result":{"items":[...],"pages":N}}
...
# ══════════════════════════════════════════════════════════════════════════════
# PLAYER — movies + TV episodes
# CRITICAL FIX: vidsrc.me embed URLs use query params, not path segments
#   Movie:   https://vidsrc.me/embed/movie?imdb=tt0104740
#   TV ep:   https://vidsrc.me/embed/tv?imdb=tt0944947&season=1&episode=1
# ══════════════════════════════════════════════════════════════════════════════
...
def play(imdb_id):
    ...
    # FIXED: vidsrc.me uses query param ?imdb= not path /embed/movie/{id}
...
# ── TV detail page (pick season/episode) ─────────────────────────────────────
...
def watch_tv(imdb_id, season, episode):
    title = request.args.get("title", "Show")
    year  = request.args.get("year", "")
    # FIXED: vidsrc.me uses query params for TV too
